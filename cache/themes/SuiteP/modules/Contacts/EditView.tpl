...
{if strlen($fields.default_interest_c.value) <= 0}
{assign var="value" value=$fields.default_interest_c.default_value }
{else}
{assign var="value" value=$fields.default_interest_c.value }
{/if}  
<input type='text' name='{$fields.default_interest_c.name}' 
id='{$fields.default_interest_c.name}' size='30' maxlength='255' value='{sugar_number_format precision=0 var=$value}' title='' tabindex='0'    >
</div>
...
{if strlen($fields.current_interest_c.value) <= 0}
{assign var="value" value=$fields.current_interest_c.default_value }
{else}
{assign var="value" value=$fields.current_interest_c.value }
{/if}  
<input type='text' name='{$fields.current_interest_c.name}' 
id='{$fields.current_interest_c.name}' size='30' maxlength='255' value='{sugar_number_format precision=0 var=$value}' title='' tabindex='0'    >
</div>
...
{else}
{assign var="field_options" value=$fields.contact_type_c.options }
{capture name="field_val"}{$fields.contact_type_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.contact_type_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.contact_type_c.name}" 
id="{$fields.contact_type_c.name}" 
title=''          
>
{if isset($fields.contact_type_c.value) && $fields.contact_type_c.value != ''}
...
<input
id="{$fields.contact_type_c.name}-input"
name="{$fields.contact_type_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.contact_type_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.contact_type_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.contact_type_c.name}-input', '{$fields.contact_type_c.name}');sync_{$fields.contact_type_c.name}()"><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
{literal}
<script>
	...
	{/literal}
			...
			{literal}
		(function (){
			var selectElem = document.getElementById("{/literal}{$fields.contact_type_c.name}{literal}");
	...
	SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.contact_type_c.name}-input');
	SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.contact_type_c.name}-image');
	SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.contact_type_c.name}');
			...
			//global variable 
			sync_{/literal}{$fields.contact_type_c.name}{literal} = function(){
				SyncToHidden();
			}
			function syncFromHiddenToWidget(){
				...
				for (i=0;i<selectElem.options.length;i++){
					...
					if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.contact_type_c.name}-input{literal}'))
						SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
				}
			}

            YAHOO.util.Event.onAvailable("{/literal}{$fields.contact_type_c.name}{literal}", syncFromHiddenToWidget);
		{/literal}
...
<input type="text" name="{$fields.contacts_contacts_1_name.name}" class="sqsEnabled" tabindex="0" id="{$fields.contacts_contacts_1_name.name}" size="" value="{$fields.contacts_contacts_1_name.value}" title='' autocomplete="off"  	 >
<input type="hidden" name="{$fields.contacts_contacts_1_name.id_name}" 
id="{$fields.contacts_contacts_1_name.id_name}" 
value="{$fields.contacts_contacts_1contacts_ida.value}">
<span class="id-ff multiple">
<button type="button" name="btn_{$fields.contacts_contacts_1_name.name}" id="btn_{$fields.contacts_contacts_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_TITLE"}" class="button firstChild" value="{sugar_translate label="LBL_ACCESSKEY_SELECT_CONTACTS_LABEL"}"
onclick='open_popup(
"{$fields.contacts_contacts_1_name.module}", 
600, 
400, 
"", 
true, 
false, 
{literal}{"call_back_function":"set_return","form_name":"EditView","field_to_name_array":{"id":"contacts_contacts_1contacts_ida","name":"contacts_contacts_1_name"}}{/literal}, 
"single", 
true
);' ><span class="suitepicon suitepicon-action-select"></span></button><button type="button" name="btn_clr_{$fields.contacts_contacts_1_name.name}" id="btn_clr_{$fields.contacts_contacts_1_name.name}" tabindex="0" title="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_TITLE"}"  class="button lastChild"
onclick="SUGAR.clearRelateField(this.form, '{$fields.contacts_contacts_1_name.name}', '{$fields.contacts_contacts_1_name.id_name}');"  value="{sugar_translate label="LBL_ACCESSKEY_CLEAR_CONTACTS_LABEL"}" ><span class="suitepicon suitepicon-action-clear"></span></button>
</span>
<script type="text/javascript">
SUGAR.util.doWhen(
		"typeof(sqs_objects) != 'undefined' && typeof(sqs_objects['{$form_name}_{$fields.contacts_contacts_1_name.name}']) != 'undefined'",
		enableQS
);
</script>
</div>
...
{if strlen($fields.pending_payment_c.value) <= 0}
{assign var="value" value=$fields.pending_payment_c.default_value }
{else}
{assign var="value" value=$fields.pending_payment_c.value }
{/if}  
<input type='text' name='{$fields.pending_payment_c.name}' 
id='{$fields.pending_payment_c.name}' size='30' maxlength='26' value='{sugar_number_format var=$value}' title='' tabindex='0'
>
</div>
...
addToValidate('EditView', 'contact_type_c', 'enum', false,'{/literal}{sugar_translate label='LBL_CONTACT_TYPE' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'pending_payment_c', 'currency', false,'{/literal}{sugar_translate label='LBL_PENDING_PAYMENT' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'default_interest_c', 'int', false,'{/literal}{sugar_translate label='LBL_DEFAULT_INTEREST' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'current_interest_c', 'int', false,'{/literal}{sugar_translate label='LBL_CURRENT_INTEREST' module='Contacts' for_js=true}{literal}' );
...
addToValidate('EditView', 'is_calculated_c', 'bool', false,'{/literal}{sugar_translate label='LBL_IS_CALCULATED' module='Contacts' for_js=true}{literal}' );
addToValidate('EditView', 'contacts_contacts_1_name', 'relate', false,'{/literal}{sugar_translate label='LBL_CONTACTS_CONTACTS_1_FROM_CONTACTS_L_TITLE' module='Contacts' for_js=true}{literal}' );
...
addToValidateBinaryDependency('EditView', 'contacts_contacts_1_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Contacts' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_CONTACTS_CONTACTS_1_FROM_CONTACTS_L_TITLE' module='Contacts' for_js=true}{literal}', 'contacts_contacts_1contacts_ida' );
